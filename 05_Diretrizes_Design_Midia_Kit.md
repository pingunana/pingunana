...
**Tipografia:**
- **Títulos, números, stickers e botões:** `Outfit` (500–800), por ter personalidade creator sem prejudicar leitura.
- **Corpo de texto e descrições:** `DM Sans` (400–700), para leitura limpa e menos aparência de template corporativo comum.
...
### Diretriz textual para marcas

O texto do mídia kit deve transmitir criatividade com previsibilidade. Evitar frases que soem como pressão comercial, ego ou ameaça indireta ("entre antes de ficar grande"). O posicionamento correto é: creator menor com bom fit, conteúdo nativo, alta interação, entregas claras, brand safety e acompanhamento legal.

Para CTAs, preferir "Vamos avaliar uma parceria?", "ações bem alinhadas" e "conteúdo mensurável" em vez de urgência ou FOMO.

### Checklist de publicação no GitHub Pages

Antes de subir nova versão, validar que o HTML publicado não depende de assets ausentes no repositório. Ícones pequenos de redes sociais devem preferencialmente ficar embutidos como data URI; quando forem arquivos externos, confirmar que `imagens/twitchlogo.png`, `imagens/instagramlogo.svg` e demais assets usados pelo HTML foram adicionados ao Git.

---

---

## 9. Hospedagem & Publicação (GitHub Pages)

- **Repositório Público:** `pingunana.github.io` (organização/usuário `pingunana`).
- **URL Pública:** `https://pingunana.github.io/` (layout limpo sem subpastas).
- **Escopo do Repositório:** Apenas o arquivo `index.html` e assets públicos serão publicados no repositório do GitHub. Todos os documentos internos de estratégia, decisões pendentes e diretrizes (`01` a `05`) permanecem estritamente privados no repositório local.

---

## 10. Histórico de Decisões

* **23/07/2026:** Definida a publicação do Mídia Kit no GitHub Pages sob o repositório `pingunana.github.io` (`https://pingunana.github.io/`), mantendo a privacidade de todos os arquivos internos de estratégia.
